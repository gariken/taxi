...
class OrderHistoryPassengerFragment : BaseFragment(), OrderHistoryPassengerView {

    companion object {
        fun newInstance(): OrderHistoryPassengerFragment {
            val args = Bundle()
            val frnt = OrderHistoryPassengerFragment()
            frnt.arguments = args
            return frnt
        }
    }
    ...
    @InjectPresenter lateinit var presenter: OrderHistoryPassengerPresenter

    private val adapter = OrderAdapter({ pos, order ->
        presenter.onOrderClicked(pos, order)
    })
    ...
    @ProvidePresenter fun providePresenter(): OrderHistoryPassengerPresenter =
            Toothpick.openScope(Scopes.APP).getInstance(OrderHistoryPassengerPresenter::class.java)
}